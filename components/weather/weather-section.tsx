    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          // Log the error in a way that doesn't trigger the error overlay
          console.log("Geolocation unavailable:", errorMessage);
        },
        {
        ...
        <CardHeader>
          ...
          {locationError && (
            <div className="mt-2 text-xs text-muted-foreground">
              <p>Note: {locationError} - Using default location</p>
            </div>
          )}
        </CardHeader>
        <CardContent>
